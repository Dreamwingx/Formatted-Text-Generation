...
def polishtext(output_dir: str, work_dir: str):
    ...
    instruction = (
        ...
        "1、标题规范：统一使用“1”、“1.1”、“1.1.1”形式的纯数字多级编号（不使用 #、##、**、- 等任何 Markdown 符号）。编号与标题文字之间用一个空格隔开。示例：\n"
        "   1 材料与方法\n"
        "   1.1 实验对象\n"
        "   1.1.1 纳入标准\n"
        "   若原文存在“#1”、“##1.1”等格式，一律转换为上述纯数字编号格式。\n\n"
        ...
        "6、只输出润色完成后的纯文本内容，不输出任何说明、解释、多余话术、修改备注。禁止输出任何 Markdown 标题符号（如 #、##、###）。\n\n"
        "【待润色文本】"
    )
